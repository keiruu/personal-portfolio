...
function DesignProject(props) {
    ...
    return (
        <motion.a href={props.link} rel="noreferrer" target="_blank">
            <motion.div className='group m-0 flex-row gap-6 rounded-3xl border-2 border-lightgray transition-all cursor-pointer hover:border-darkgreen px-8 py-12 justify-center max-w-[300px] min-h-[405px]'>
                ...
                <div className='flex flex-col gap-6'>
                    <div className='flex justify-between mt-4'>
                        ...
                        <button className='bg-lightgreen text-lg px-[1rem] lg:px-[0.9rem] text-darkgreen rounded-full transition-all group-hover:text-[#6D785B] group-hover:rotate-[-45deg]'><FontAwesomeIcon icon={faArrowRight} /></button>
                    </div>
  ...
  return (
    <motion.div className="flex flex-col items-center justify-center w-full gap-12 pt-32">
        <motion.div variants={container} ref={ref} initial="hidden" animate={animationControl} className="flex flex-col gap-12 mb-20 px-4">
            ...
            <h1 className='font-bold text-primary text-xl lg:text-3xl lg:my-4'>Design Projects</h1>
            <motion.div className='flex gap-10 justify-between'>
